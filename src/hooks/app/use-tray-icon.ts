  ...
  const scheduleTrayIconUpdate = useCallback((
    ...
    trayUpdateTimerRef.current = window.setTimeout(() => {
      ...
      const setTrayTitle = (title: string | null) => {
        if (setTitleFn) {
          return setTitleFn(title as string)
        }
      ...
      renderTrayBarsIcon({
        ...
        .then(async (img) => {
          ...
          if (typeof (tray as any).setTooltip === "function") {
            const tooltipSetter = (tray as any).setTooltip.bind(tray)
            // If tooltip is null, default to the label string joining if it existed or clear
            await tooltipSetter(tooltipText ?? null).catch(() => { })
          }
        })
        .catch((e) => {
